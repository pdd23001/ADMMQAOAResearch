...
class ADMMOptimizer(OptimizationAlgorithm):
    ...
    **References:**
    ...
    def _update_x0(self, op1: QuadraticProgram) -> np.ndarray:
        ...
        """
        print("New Iteration")
        # Get the full result from the QUBO solver.
        result = self._qubo_optimizer.solve(op1)
        samples = result.samples  # List[SolutionSample]

        # Filter the samples based on feasibility against clique constraints.
        feasible_samples = []
        for sample in samples:
            x_bits = np.asarray(sample.x, dtype=int)  # Convert to integer 0/1
            is_feasible = True
            # Check each inequality constraint (expected to be of the form: x[u] + x[v] <= 1)
            print("Checkin Sample:",x_bits)
            for constr in self._state.op.linear_constraints:
                if constr.sense == Constraint.Sense.LE:
                    # Convert the linear expression to a coefficient array.
                    coeffs = constr.linear.to_array()
                    print("Variable vals:", coeffs)
                    print()
                    lhs_val = np.dot(coeffs, x_bits)
                    print("Summation of two:",lhs_val)
                    print()
                    if lhs_val > 1:
                        is_feasible = False
                        break
            if is_feasible:
                feasible_samples.append(x_bits)
                print("Feasible samples:", feasible_samples)
            print()

        # Evaluate each feasible sample using the true objective:
        # For max clique, objective = - (sum of selected nodes)
        best_obj = +np.inf
        best_sample = None
        if feasible_samples:
            for x_bits in feasible_samples:
                obj_val = -np.sum(x_bits)
                if obj_val < best_obj:
                    best_obj = obj_val
                    best_sample = x_bits

        if best_sample is None:
            # Fallback to the solver's x if something goes wrong.
            best_sample = np.asarray(result.x, dtype=int)
        # Prepare the full binary solution vector.
        x0_all_binaries = np.zeros(len(self._state.binary_indices), dtype=int)
        # Update only the variables corresponding to the step1 (QUBO) indices.
        print("Best Sample:", best_sample)
        print("Objective:",best_obj)
        x0_all_binaries[self._state.step1_relative_indices] = best_sample
