...
class OptimizationAlgorithm(ABC):
    ...
    def _interpret_samples(
        ...
        # print("Sorted Samples:",sorted_samples)
         # --- New code: filter out infeasible samples --- (I don't think this was needed as filtering already done)
        feasible_samples = [s for s in sorted_samples if s.status == OptimizationResultStatus.SUCCESS]
        if feasible_samples:
            best_sample = feasible_samples[0]
        else:
            raise(QiskitOptimizationError("No feasible solution found."))
        best_raw = raw_samples[index[tuple(best_sample.x)]]
